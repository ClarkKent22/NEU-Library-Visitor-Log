...
if (!empty($search)) {
    $sql .= " AND (
        ...
        OR block_reason LIKE '%$search%'
        OR DATE_FORMAT(date_blocked, '%b %e, %Y') LIKE '%$search%'
        OR DATE_FORMAT(date_blocked, '%h:%i %p') LIKE '%$search%'
        OR 'Access Restricted' LIKE '%$search%'
        OR 'Restore Access' LIKE '%$search%'
    )";
}
