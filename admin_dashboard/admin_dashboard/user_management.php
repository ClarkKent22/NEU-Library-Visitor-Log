...
if (isset($_GET['ajax'])) {
    ...
    if ($search) {
        ...
        $roleExpr = ($tableType === 'students') ? "'Student'" : "u.role";
        $sql .= " AND (
            u.firstName LIKE '%$searchVal%' 
            OR u.lastName LIKE '%$searchVal%' 
            OR $idCol LIKE '%$searchVal%'
            OR d.departmentName LIKE '%$searchVal%'
            OR $roleExpr LIKE '%$searchVal%'
            OR u.status LIKE '%$searchVal%'
            OR DATE_FORMAT(h.last_login, '%h:%i %p') LIKE '%$searchVal%'
            OR (h.last_login IS NOT NULL AND 'Logged in today' LIKE '%$searchVal%')
            OR (h.last_login IS NULL AND 'Not logged in' LIKE '%$searchVal%')
        )";
    }
        ...
        <ul class="nav nav-tabs px-3 pt-2 bg-light border-bottom" id="userTabs">
            ...
            <li class="nav-item">
                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#employees-pane" id="employeeTabBtn">
                    <i class="bi bi-person-badge-fill me-1"></i> Employee / Faculty / Admin
                </button>
            </li>
        </ul>
