...
if (isset($_POST['check_user'])) {
    ...
    if ($found_id) {
        ...
        // 3. If NOT blocked, allow entry flow (users can log multiple times per day)
        $_SESSION['user_id'] = trim($found_id);
...
$dept_list = [];
$dept_res = $conn->query("SELECT departmentID, departmentName FROM departments ORDER BY departmentName ASC");
if ($dept_res) {
    $dept_list = $dept_res->fetch_all(MYSQLI_ASSOC);
}

$google_reg_info = $_SESSION['google_reg_info'] ?? null;
if ($google_reg_info) {
    unset($_SESSION['google_reg_info']);
}
?>
...
<body class="theme-dark">
    ...
    <div class="kiosk-shell">
        <div class="kiosk-hero">
            ...
            <div class="kiosk-metrics">
                ...
                <div class="metric">
                    <div class="label">Philippine Date & Time</div>
                    <div class="value" id="phDateTime">Loading...</div>
                </div>
                <div class="metric">
        ...
        <div class="kiosk-card">
            <div class="divcardmain" style="box-shadow:none; border:none; padding:0; background:transparent;">
                ...
                <a href="entrynavpages/google_auth_start.php" class="btn btn-light w-100 d-flex align-items-center justify-content-center gap-2 border mb-4" style="border-radius: 10px; font-weight: 700; color: #1a1a1a;">
                    <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" style="height: 18px; width: 18px;">
                ...
                <form method="POST" id="loginForm">
                    ...
                    <div class="mb-3">
                        ...
                        <label class="small fw-bold text-muted d-block mb-1">STUDENT / Employee ID</label>
                        <input type="text" name="user_id" id="user_id" class="form-control border-2" placeholder="e.g. 10000001" inputmode="numeric" maxlength="20" style="border-radius: 10px;">
                    </div>
    ...
    <div class="modal fade" id="publicAddUserModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                ...
                <div style="text-align: center; padding: 30px 20px 20px; border-bottom: 1px solid #e0e0e0;">
                    <img src="assets/neu.png" alt="NEU Logo" style="width: 70px; height: 70px; margin-bottom: 15px; object-fit: contain;">
                    <div style="font-size: 0.75rem; font-weight: 700; letter-spacing: 2px; color: #888; text-transform: uppercase; margin-bottom: 8px;">New Era University</div>
                    <h2 style="font-size: 1.75rem; font-weight: 800; margin: 0 0 8px 0; color: #fff;">Library Registration</h2>
                    <p style="font-size: 0.9rem; color: #aaa; margin: 0;">Complete your profile to access the library</p>
                </div>
                <form id="publicAddUserForm" enctype="multipart/form-data">
                    <div class="modal-body py-4">
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted">User Type</label>
                            <select name="user_type" id="publicAddUserType" class="form-select" required>
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted">Student / Employee ID Number</label>
                            <input type="text" name="identifier" class="form-control" placeholder="e,g. 1000050" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div id="publicAddUserMsg" class="small text-danger" style="display:none;"></div>
                    </div>
    ...
    <script>
        ...
        loginForm.addEventListener('submit', function(e) {
            ...
            } else if (idValue && (idValue.length < 4 || idValue.length > 20)) {
                e.preventDefault();
                showError("Format Error: ID must be 4 to 20 digits.");
        ...
        const publicAddUserForm = document.getElementById('publicAddUserForm');
        if (publicAddUserForm) {
            publicAddUserForm.addEventListener('submit', function(e) {
                e.preventDefault();
                const msg = document.getElementById('publicAddUserMsg');
                msg.style.display = 'none';
                
                try {
                    // Get form field values safely
                    const firstNameEl = publicAddUserForm.querySelector('[name="firstName"]');
                    const lastNameEl = publicAddUserForm.querySelector('[name="lastName"]');
                    const emailEl = publicAddUserForm.querySelector('[name="institutionalEmail"]');
                    const identifierEl = publicAddUserForm.querySelector('[name="identifier"]');
                    const departmentEl = publicAddUserForm.querySelector('[name="departmentID"]');
                    const userTypeEl = publicAddUserForm.querySelector('[name="user_type"]');
                    
                    // Validate all elements exist
                    if (!firstNameEl || !lastNameEl || !emailEl || !identifierEl || !departmentEl || !userTypeEl) {
                        msg.textContent = 'Form error: Missing fields. Please refresh the page.';
                    ...
                    const departmentID = departmentEl.value.trim();
                    const userType = userTypeEl.value;
                    
                    // Validate fields are not empty
                    if (!firstName || !lastName || !email || !identifier || !departmentID) {
                    ...
                    if (!email.endsWith('@neu.edu.ph')) {
                        msg.textContent = 'Only @neu.edu.ph emails are allowed.';
                        msg.style.display = 'block';
                        return;
                    }
                    ...
                    if (!/^\d{4,20}$/.test(identifier)) {
                        msg.textContent = 'ID must be 4-20 digits.';
                        msg.style.display = 'block';
                        return;
                    }
                    
                    // Create FormData and submit
                    const formData = new FormData(publicAddUserForm);
                    
                    fetch('entrynavpages/register_user.php', {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => {
                    ...
                    .then(text => {
                        ...
                        console.log('Response from server:', trimmedText);
                        
                        if (trimmedText === 'success') {
                            publicAddUserForm.reset();
                            const modalEl = document.getElementById('publicAddUserModal');
                            bootstrap.Modal.getOrCreateInstance(modalEl).hide();
                            showError('Registration successful! You can now log in with your ID/Email.');
                        } else {
                            // Display server error message
                            msg.textContent = trimmedText || 'Registration failed. Please try again.';
                            msg.style.display = 'block';
                        }
                    })
                    .catch(err => {
                    ...
                    msg.textContent = 'An error occurred. Please check the browser console.';
                    msg.style.display = 'block';
                }
            });
        }

        // Auto-open registration modal when redirected from Google sign-in for new user.
        <?php if (!empty($google_reg_info) && isset($_GET['autoAddUser']) && $_GET['autoAddUser'] == '1'): ?>
        document.addEventListener('DOMContentLoaded', function() {
            const modalEl = document.getElementById('publicAddUserModal');
            const modal = new bootstrap.Modal(modalEl);
            const form = publicAddUserForm;
            if (form) {
                form.querySelector('[name="firstName"]').value = <?php echo json_encode($google_reg_info['firstName'] ?? ''); ?>;
                form.querySelector('[name="lastName"]').value = <?php echo json_encode($google_reg_info['lastName'] ?? ''); ?>;
                form.querySelector('[name="institutionalEmail"]').value = <?php echo json_encode($google_reg_info['email'] ?? ''); ?>;
            }
            modal.show();
        });
        <?php endif; ?>
        ...
        function updatePhilippineDateTime() {
            ...
            const phTimeString = now.toLocaleTimeString('en-US', {
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit',
                hour12: true,
            ...
            const target = document.getElementById('phDateTime');
            if (target) {
                target.textContent = `${phDateString} ${phTimeString}`;
            }
        }
